...
<template>
  ...
  >
    <span
      class="mt-px size-[22px] flex-none [&>svg]:size-full"
      :class="v.ac"
      aria-hidden="true"
    >
    ...
    <div class="min-w-0">
      <p
        v-if="title"
        class="m-0 mb-0.5 font-[family-name:var(--font-display)] text-[1.125rem] font-extrabold tracking-[-0.015em]"
        :class="titleColor"
      >
        {{ title }}
      </p>
      <div
        class="m-0 text-[0.9375rem] leading-[1.55]"
        :class="bodyColor"
      >
        <slot />
      </div>
