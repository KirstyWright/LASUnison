...
<template>
  ...
  >
    ...
    <!-- media -->
    <div
      class="relative flex-none overflow-hidden bg-[var(--surface-sunken)]"
    ...
    >
      ...
      <span
        v-if="post.urgent"
        class="absolute top-3 left-3 inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] bg-[var(--emergency)] px-2.5 py-1.5 font-[family-name:var(--font-sans)] text-[0.75rem] leading-none font-bold tracking-[0.04em] text-white uppercase shadow-[var(--shadow-sm)]"
      >
        <UiIcon
          name="alert"
          :size="13"
    ...
    <!-- body -->
    <div
      class="flex flex-1 flex-col gap-2.5 p-5"
      :class="featured ? 'lg:p-8 lg:justify-center' : 'pb-6'"
    >
      <span
        class="font-[family-name:var(--font-sans)] text-[0.8125rem] leading-none font-bold tracking-[0.08em] uppercase"
        :style="{ color: meta.accent }"
        aria-hidden="true"
      >{{ post.category }}</span>

      <h3
        class="m-0 font-[family-name:var(--font-display)] font-extrabold tracking-[-0.015em] text-[var(--text-strong)] group-hover:text-[var(--brand-primary)]"
        :class="featured
      ...
      <p
        class="m-0 flex items-center gap-2 font-[family-name:var(--font-mono)] text-[0.8125rem] text-[var(--text-muted)]"
        aria-hidden="true"
      >
      ...
      <div class="mt-auto flex items-center gap-3 pt-3">
        ...
        >{{ post.topic }}</UiBadge>
        <span
          class="ml-auto inline-flex items-center gap-1 text-[0.875rem] font-bold text-[var(--brand-primary)]"
          aria-hidden="true"
        >
